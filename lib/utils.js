...
U.formatingQuery =  (queries, curQuestion, inquirerQuestions, answers) => {
    ...
    queries = queries.map(q =>{
        if(curQuestion == inquirerQuestions['initialQuestion']){ 
            ...
            let val = (U.replacingPlaceHolders(q, answers )).replace('= null', 'IS null')
...
U.processQuery =  function(pool, query) {
    console.log(U.c('Processing query...'), query)
...
U.subQueries = {
    ...
    'View department by id': 'SELECT * FROM department;',
}
...
{ //defining query options
    ...
    U.queryOptions['View total utilized budget of a department'] = `SELECT department, SUM(salary) FROM role WHERE department = '[department]' GROUP BY department;` 
    U.queryOptions['Add a department'] = [ `INSERT INTO department (name) VALUES ('[name]');`, U.subQueries['View department by id'] ]
    U.queryOptions['Add a role']= ` INSERT INTO role (title, salary, department_id, department) VALUES ('[title]', [salary], [department_id], (SELECT name FROM department WHERE id = [department_id])); `
    ...
    U.queryOptions['Update an employee role'] = [`
        ...
        U.queryOptions['View employee by id']
    ]
    U.queryOptions['Update employee manager'] = [
        `UPDATE employee SET manager_id = [new_value] WHERE id = [employee_id];`,
        U.queryOptions['View employee by id']
    ]
    U.queryOptions['Remove a department'] = [`
        BEGIN;
        UPDATE employee SET department_id = NULL WHERE department_id = 4;
        DELETE FROM role WHERE department_id = 4;
    ...
    tables = tables || {
        ...
        employee: {
            query: `CREATE TABLE IF NOT EXISTS employee (
                ...
                position VARCHAR(30) REFERENCES role(title) ON DELETE SET NULL,
...
U.getChoices = async (obj) => {
    ...
    otherValsObj = otherValsObj || { 'name': 'NULL', 'value': 'NULL' }
    ...
    return {
        ...
        'Add a role': [
            ...
            { name: 'department_id', type: 'list', message: 'Under what department do you want to add it:', 
                choices: async  () => await U.getChoices({pool, query: q['View all departments'], keysArray: ['name', 'id'], otherValsObj: {}})
            },
        ],
        'Update an employee role': [
            ...
            { name: 'new_role', type: 'list', message: 'Select a role:', 
                choices: async () => (await U.getChoices({ pool, query: U.queryOptions['View all roles'], keysArray: ['title', 'title'] , otherValsObj: {}, debug: true}))
            },
        ],
        ...
        'Remove a department': [
            { name: 'department_id', type: 'input', message: 'Deparment Id:',
                validate: async (val) => {
                    ...
                    const employees = await U.getChoices({pool, query: q['View all employees by department'], keysArray: ['first_name', 'id'], otherValsObj: {}, placeHolderVal: {department_id: val} });
        ...
        'Remove a role': [
            { name: 'role_id', type: 'input', message: `What role do you want to ${U.c('remove','r')}:`, 
                validate: async (val) => {
                    if(val.length == 0) { return 'Please enter a role id'}
                    const employees = await U.getChoices({pool, query: q['View all employees by role'], keysArray: ['first_name', 'id'], otherValsObj: {}, placeHolderVal: {role_id: val} });
                    if (employees.length > 0) {
