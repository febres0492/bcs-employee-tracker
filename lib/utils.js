...
U.getChoices = async (obj) => {
    ...
    otherValsObj = otherValsObj || {}
    otherValsObj = Array.isArray(otherValsObj) ?  otherValsObj : [otherValsObj]
...
U.validateEmployeeId = async (obj) => {
    let { pool, val, queryOptions } = obj;
    console.log(U.c('\nval', 'r'), val, typeof val)
...
U.actitonMessages = {
    'exitText': U.c('< Exit program >', 'y'),
    'exit': 'Loading main memu...',
    'exit_1' : `\nCannot remove department. Move the employees to another department first`,
    'exit_2': `\nCannot remove role. Move the employees to another role first`,
}

U.getInquirerQuestions = (obj) => {
    let { pool } = obj
    let q = U.queryOptions

    return {
        'initialQuestion': [
            {   name: 'options', type:'list', message: `Options:`, choices: [...Object.keys(q), U.actitonMessages['exitText']] }
        ],
        'View employee by id': [
            { name: 'employee_id', type: 'input', message: 'Employee id:', 
                validate: (val) => {
                    if((val+'').trim().toLowerCase() == 'exit') { return true }
                    return U.validateEmployeeId({pool, val, 'queryOptions': U.subQueries})
                },
        ...
        'View all employees by manager': [
            { name: 'manager_id', type: 'list', message: 'Select a manager:',
                choices: async  () => {
                    return await U.getChoices({
                        pool, 
                        query: q['View all managers'], 
                        keysArray: ['first_name, last_name', 'id'], 
                        otherValsObj: [{ 'name': 'Employee without manager', 'value': 'NULL' }, { 'name': U.c('< Exit >','y'), 'value': 'exit' }] 
                    })
                }
            }
        ],
        'View all employees by department': [
            { name: 'department_id', type: 'list', message: 'Select a department:',
                choices: async  () => {
                    return await U.getChoices({
                        pool, 
                        query: q['View all departments'], 
                        keysArray: ['name', 'id'],
                        otherValsObj:  { 'name': U.c('< Exit >','y'), 'value': 'exit' }
                    })
                }
            }
        ],
        ...
        'Add an employee': [
            ...
            { name: 'last_name', type: 'input', message: `Enter ${U.c('Last Name')}:`,
                ...
                filter: (val) => U.capAll(val.trim()),
                when: (answers) => !Object.values(answers).some(val => val.toLowerCase() == 'exit')
            },
            ...
            { name: 'manager_id', type: 'list', message: 'Under what Manager:', 
                choices: async  () => { 
                    return await U.getChoices({
                        ...
                        query: q['View all managers'], 
                        keysArray: ['first_name, last_name', 'id'], 
                        otherValsObj: { 'name': 'NULL', 'value': 'NULL' }
                    })
                },
                when: (answers) => !Object.values(answers).some(val => val.toLowerCase() == 'exit')
            },
        ...
        'Update an employee role': [
            ...
            { name: 'new_role', type: 'list', message: 'Select a role:', 
                choices: async () => { 
                    return await U.getChoices({ 
                        pool, 
                        query: U.queryOptions['View all roles'], 
                        keysArray: ['title', 'title'],
                        otherValsObj:  { 'name': U.c('< Exit >', 'y'), 'value': 'exit' }
                    })
                },
                when: (answers) => !Object.values(answers).some(val => val.toLowerCase() == 'exit')
            ...
            { name: 'new_value', type: 'list', message: 'Select a manager:',
                choices: async  () => { 
                    return await U.getChoices({ 
                        ...
                        query: q['View all managers'], 
                        keysArray: ['first_name, last_name', 'id'], 
                        otherValsObj: [{ 'name': 'NULL', 'value': 'NULL' }, { 'name': U.c('< Exit >', 'y'), 'value': 'exit' }]  
                    })
                },
                when: (answers) => !Object.values(answers).some(val => val.toLowerCase() == 'exit')
            }
        ...
        'Remove a department': [
            { name: 'department_id', type: 'input', message: `Deparment Id to be ${U.c('removed','r')}:`,
                validate: async (val) => {
                    if(val.trim().toLowerCase().indexOf('exit') > -1) { return true }
                    if(val.length == 0) { return 'Please enter a department id'}
                    if(isNaN(+val)) { return 'Please enter a valid number'}
                    return true
                },
                filter: async (val) => {
                    if(val.trim().toLowerCase().indexOf('exit') > -1) { return val }
                    ...
                    if (employees.length > 0) { return 'exit_1' }
                    return val.trim()
                }
            }
        ],
        'Remove a role': [
            { name: 'role_id', type: 'input', message: `Role Id to be ${U.c('removed','r')}:`, 
                validate: async (val) => {
                    if(val.trim().toLowerCase().indexOf('exit') > -1) { return true }
                ...
                filter: async (val) => {
                    ...
                    if (employees.length > 0) { return 'exit_2' }
                    return val.trim()
                }
            }
        ],
        'Remove an employee': [
            { name: 'employee_id', type: 'input', message: `Employee Id to be ${U.c('removed','r')}:`,
                validate: (val) => {
                    if(val.trim().toLowerCase().indexOf('exit') > -1) { return true }
                    U.validateEmployeeId({pool, val, 'queryOptions': U.subQueries})
                },
                filter: async (val) => {
                    if(val.trim().toLowerCase().indexOf('exit') > -1) { return val }
                    const employees = await U.getChoices({pool, query: q['View employee by id'], keysArray: ['first_name', 'id'], placeHolderVal: {employee_id: val} }) //
                    if (employees.length > 0) { return 'exit_2' }
                    return val.trim()
                }
            },
